...
EDIT of KineticNCBeamicon2 on 2025-07-16 for Masso Compatibility

Masso Tool Changes
  https://www.masso.com.au/tool-change-commands/
  https://docs.masso.com.au/supported-g-codes


ISSUE: Masso expects "T2 M6", but the original postprocessor emits "M6 T2". This fixes it. The reasoning is big machines can prepare for the tool change while still cutting with the old tool, and issuing T2 first prepares for a faster M6 tool change.
    ...
    if hasattr(pathobj, "Group"):  # We have a compound or project.
        ...
        for c in PathUtils.getPathWithPlacement(pathobj).Commands:
            ...
            if command == "M6":
                ...
                out += '(begin MASSO toolchange)\n'

                # Stop spindle & coolant
                for line in TOOL_CHANGE.splitlines(True):
                    out += linenumber() + line

                # Emit correct Masso tool change, T2 *before* M6
                if tool_num is not None:
                    out += linenumber() + f"T{int(tool_num)} M6\n"
                else:
                    raise Exception('M6 received, but tool_num was None')

                # The default path below, (if len(outstring) >= 1) was
                # previously used to emit `M6 T2`. Since we're emitting it
                # ourselves, let's skip to the next loop iteration to avoid
                # printing it twice.
                continue
